...
class UserForm {
	...
	protected function validForUpdate(array $input)
	{
		...
		$uniqueUserName = $this->validator->with($input)->updateUnique('username', $input['id'],'id')->passes();
		$uniqueEmail = $this->validator->with($input)->updateUnique('email', $input['id'],'id')->passes();
