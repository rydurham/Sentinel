...
<table>
	...
	@foreach ($groups as $group)
		<tr>
			...
			<td>
				<button onClick="location.href='{{ route('sentinel.groups.edit', [$group->id]) }}'">Edit</button>
				<button class="action_confirm {{ ($group->name == 'Admins') ? 'disabled' : '' }}" type="button" data-token="{{ Session::getToken() }}" data-method="delete" href="{{ route('sentinel.groups.destroy', [$group->id]) }}">Delete</button>
			 </td>
		</tr>	
	@endforeach
	</tbody>
