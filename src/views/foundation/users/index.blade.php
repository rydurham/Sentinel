				...
				<tr>
					...
					<td>
						...
						@if ($user->status != 'Suspended')
							<button class="button small" type="button" onClick="location.href='{{ action('Sentinel\UserController@suspend', array($user->id)) }}'">Suspend</button> 
						@else
						...
						@endif
						<button class="button small alert action_confirm" href="{{ action('Sentinel\UserController@destroy', array($user->id)) }}" data-token="{{ Session::getToken() }}" data-method="delete">Delete</button>
					</td>
				</tr>
			@endforeach
		</tbody>
